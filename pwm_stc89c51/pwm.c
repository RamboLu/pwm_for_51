...
void pwm_set(uint freq, uint duty_ratio)
{
	...
	if(freq>1000)		//1000HZ周期为1000us
	{
		//时间基5us
		base_time=5;
		TH0=(65536-5)/256;//16位定时器自动不可重装载值
		TL0=(65536-5)%256;
	}
	...
	num-=8;//补偿定时器累积误差 需要实测，stc15单片机速度较快所以误差较小；stc89c51可能需要改
	//不一定是8，需要接了示波器，或者逻辑分析仪看脉宽。
...
{
	TH0=(65536-base_time)/256;
	TL0=(65536-base_time)%256;
	n++;
